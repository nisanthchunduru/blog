...
export default function ChirpsFeed({
  chirps,
  title = 'Chirps',
  limit,
  ...
  const publishedChirps = sortBy(chirps.filter(chirp => !chirp.draft), 'publishedDate', 'desc')
  const chirpsToRender = typeof limit === 'number' ? publishedChirps.slice(0, limit) : publishedChirps
  const chirpsByMonthAndYear = chirpsToRender.reduce<{ monthAndYear: string; chirps: Chirp[] }[]>((groups, chirp) => {
